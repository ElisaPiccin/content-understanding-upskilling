"""
This script is exported from the View code snippet in the Azure portal.
...
FIXES:
Client authentication with get_bearer_token_provider() instead of using the subscription key.

DISCLAIMER:
client_quickstart_NEW.py is robust to changes in the result URL because it uses the service-provided operation-location for polling, 
while the analyze_doc scripts hardcode the result URL and must be updated if the API changes. 
This makes the client approach more future-proof and less sensitive to backend changes.

USAGE:
For testing the script run: 
python client_quickstart_NEW.py

or, if you want to write the output to a file, run:
python client_quickstart_NEW.py > output.txt
"""
...
def main():
    ...
    settings = Settings(
        ...
        api_version=os.getenv("AZURE_CU_API_VERSION_NEW", "2025-05-01-preview"),
        ...
        analyzer_id=os.getenv("AZURE_CU_ANALYZER_NAME_NEW"),
    ...
    print(f"Analyzing file {settings.file_location} \nwith analyzer: {settings.analyzer_id}")
